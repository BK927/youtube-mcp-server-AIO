...
Items that exceed a page's byte budget remain available through `page.next_cursor`, including on the last upstream page. Follow it with the same filters and options; `limit` may change. The server consumes the retained snapshot before using the upstream next-page token. Each snapshot is limited to 512 KiB, with 64 process-local entries. The Firestore profile also stores snapshots in `youtube_response_pages` for cross-instance continuation; provisioning enables TTL on `delete_at`. Local cache eviction or restart returns an explicit cursor error rather than silently skipping items. If even one item cannot fit safely, the operation returns a structured error.

For video metadata, `captionAvailable` is a compatibility alias for the YouTube Data API's `contentDetails.caption` flag. `captionAvailability` identifies that source and preserves a missing flag as `reportedAvailable=null`. Metadata does not probe transcript providers: `transcriptRetrievability=unknown`. A false API flag must not prevent a `view=transcript` attempt, including for automatic captions.

For video metadata, `max_chars` limits descriptions and preserves IDs, titles, channel references, dates, durations, statistics, provider and completeness. Shortened content has a warning and truncation metadata. Trending returns a compact projection (identity, title, channel, publication time, duration, statistics, one thumbnail and provider); use `youtube_video_get` for full details. Channel searches include only the channel ID/title, and search totals are explicitly approximate. Exact handles and usernames that resolve to no channel return `NOT_FOUND` without substituting a search result. Supported filters are listed at `youtube://schema/youtube_search`; the global language filter is `relevance_language`.
